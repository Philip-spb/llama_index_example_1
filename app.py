...
def chat_with_profile(session_id, user_query, chat_history):
    ...
    try:
        ...
        if hasattr(response, "response"):
            answer_text = str(getattr(response, "response") or "").strip()
        else:
            answer_text = str(response).strip() if response is not None else ""

        if answer_text.lower() in {"", "none", "null", "empty response"}:
            answer_text = (
                "I couldn't generate an answer for this question. "
                "Please try rephrasing it."
            )
        logger.info("Chat response generated (chars=%s)", len(answer_text))
        return history + [
            {"role": "user", "content": user_query},
